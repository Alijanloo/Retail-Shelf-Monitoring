...
class ShelfModel(Base):
    ...
    detections = relationship(
        "DetectionModel", back_populates="shelf", cascade="all, delete-orphan"
    )
    alerts = relationship(
        "AlertModel", back_populates="shelf", cascade="all, delete-orphan"
    )

    __table_args__ = (Index("idx_shelf_store_active", "store_id", "active"),)
...
class PlanogramModel(Base):
    ...
    shelf_id = Column(
        String(255),
        ForeignKey("shelves.shelf_id", ondelete="CASCADE"),
        primary_key=True,
    )
