...
class PlanogramItem(BaseModel):
    ...
    confidence: float = Field(default=1.0, ge=0, le=1, description="Detection confidence in reference")
...
class PlanogramRow(BaseModel):
    ...
    items: List[PlanogramItem] = Field(..., min_items=1, description="Items in this row")
    ...
    def items_sorted_by_x(cls, v):
        ...
        for idx, item in enumerate(sorted_items):
            if item.item_idx != idx:
                raise ValueError(f"Item indices must be sequential from 0; found {item.item_idx} at position {idx}")
        return sorted_items
...
class PlanogramGrid(BaseModel):
    rows: List[PlanogramRow] = Field(..., min_items=1, description="Rows in the planogram")
    ...
    def rows_sorted_by_y(cls, v):
        ...
        for idx, row in enumerate(sorted_rows):
            if row.row_idx != idx:
                raise ValueError(f"Row indices must be sequential from 0; found {row.row_idx} at position {idx}")
        return sorted_rows
...
class ClusteringParams(BaseModel):
    row_clustering_method: str = Field(default="dbscan", description="Clustering method (dbscan, kmeans)")
    eps: float = Field(default=15.0, gt=0, description="DBSCAN epsilon parameter")
    min_samples: int = Field(default=2, ge=1, description="DBSCAN min_samples parameter")
...
class Planogram(BaseModel):
    ...
    clustering_params: ClusteringParams = Field(..., description="Parameters used for grid generation")
