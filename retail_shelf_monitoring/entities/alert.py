...
class Alert(BaseModel):
    ...
    expected_sku: Optional[str] = Field(None, description="Expected SKU at this position")
    detected_sku: Optional[str] = Field(None, description="Actually detected SKU (for misplacement)")
    priority: Priority = Field(default=Priority.MEDIUM, description="Alert priority")
    first_seen: datetime = Field(..., description="First timestamp alert condition detected")
    last_seen: datetime = Field(..., description="Last timestamp alert condition detected")
    confirmed: bool = Field(default=False, description="Whether alert is confirmed by staff")
    ...
    evidence_paths: List[str] = Field(default_factory=list, description="Paths to evidence images")
    consecutive_frames: int = Field(default=1, ge=1, description="Number of consecutive frames with issue")
